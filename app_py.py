...
st.write("Upload a **retinal fundus image** to detect DR severity using EfficientNetB0 and XAI.")

uploaded_file = st.file_uploader("Upload a Retinal Image", type=["jpg", "jpeg", "png"])

if uploaded_file is not None:
    image = Image.open(uploaded_file).convert('RGB')
    st.image(image, caption="Uploaded Image", use_column_width=700)
    ...
    try:
        ...
        st.image(gradcam_overlay, caption="Grad-CAM++", use_column_width=700)
    ...
    try:
        ...
        st.image(lime_img, caption="LIME Explanation", use_column_width=700)
    except Exception as e:
